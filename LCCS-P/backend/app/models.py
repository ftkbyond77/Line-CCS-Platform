...
class Ticket(Base):
    ...
    # 🌟 พระเอกหลัก: Workflow Stage ที่พนักงานควบคุมเอง
    stage = Column(String, default="รับ order") 
    
    # 🔴 ตัวดักจับแชทค้างตอบ (SLA)
    is_replied = Column(Boolean, default=False)   
    
    created_at = Column(DateTime, default=bkk_now)
    updated_at = Column(DateTime, default=bkk_now, onupdate=bkk_now)

    account = relationship("Account", back_populates="tickets")
    messages = relationship("Message", back_populates="ticket")
    schedules = relationship("Schedule", back_populates="ticket")

# Message และ Schedule schema คงเดิมไม่เปลี่ยนแปลง...
class Message(Base):
    ...
    timestamp = Column(DateTime, default=bkk_now)
